        ...
        <SuggestionBlock>
            ...
            <S.ScriptContainer>
                <S.SuggestionBox>
                    ...
                    {Ftoggle ? <ul style={{marginTop:10}}>
                         {list_top_50.map(v => {
                            return <S.SuggestionSubBox key={v[0]}>
                               <span className="suggestion-word">{v[0]}</span>
                               <span className="suggestion-word">{v[1]}</span>
                            </S.SuggestionSubBox>
                         })}
                     </ul> : null
                    }
                ...
                <S.SuggestionButton onClick={() => setStoggle(!Stoggle)}>동의어 리스트 {Stoggle ? '접기' : '확인'}</S.SuggestionButton>
                    <S.SuggestionContentBox style={{marginTop:10}}>
                        동의어를 클릭하시면 복사됩니다.
                    </S.SuggestionContentBox>
                    {Stoggle ? <ul>
                        {synonym_list.map(v => {
                            return <S.SynonymWordBox key={v.voca_record.id}>
                                <span className="word">{v.voca_record.word}</span>
                                <img src="https://d2mkevusy1mb28.cloudfront.net/mobile/btn-dropdown-nor%403x.png" alt="arrow-up"/>
                            </S.SynonymWordBox>
                        })} </ul>
                    : null }
